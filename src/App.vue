...
<template>
  ...
  <div v-if="userName.length > 0">Usuario: {{ userName }}</div>
  <div v-else class="mx-auto p-3 lg:w-1/4 border-2 border-gray-800 rounded shadow shadow-gray-700">
    <form @submit.prevent="addUsername()" class="flex flex-col gap-3 font-Poppins justify-center items-center p-4">
      ...
      <input type="text" placeholder="escribir" class="font-Poppins text-gray-700 indent-1  outline-green-600 rounded" v-model="newUserName">
      <button class="text-white bg-green-600 hover:bg-green-700 duration-500 ease-in p-1 font-Poppins rounded">Guardar</button>
    </form>
